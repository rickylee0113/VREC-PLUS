...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ onTimeUpdate, videoRef, isActive = true }) => {
  ...
  useEffect(() => {
      ...
      const handleWheel = (e: WheelEvent) => {
          if (!isActive) return; // Disable when inactive
  ...
  }, [videoRef, onTimeUpdate, videoSrc, isActive]); // Re-attach when video source or active state changes
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (!isActive) return; // Do not respond to keys if inactive
